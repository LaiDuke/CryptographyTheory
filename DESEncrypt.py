...
def process(plain_str, all_round_key):
    ...
    for i in range(0, 16):
        ...
        left_str = xor(left_str, F_funtion(right_str, all_round_key[i]))
...
def DES_decrypt(cipher_hex, key_hex):
    key_bin = generate_key(key_hex)[::-1]
    plain_hex = bin2hex(process(cipher_hex, key_bin))
    print("Plain Text : ", plain_hex)
    return plain_hex
